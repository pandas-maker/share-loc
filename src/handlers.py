...
TEXTS = {
    'am': {
        ...
        'error': "❌ ስህተት ተፈጥሯል",
        'cancel': "❌ ተሰርዟል። ወደ መጀመሪያ ምናሌ ለመመለስ /start ይጻፉ።",
        'location_history': "📊 የቦታ ታሪክ\n\nገና ምንም ታሪክ የለም።",
    },
    'en': {
        ...
        'cancel': "❌ Cancelled. Type /start to return to the main menu.",
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "share_my_location":
        ...
        link_text = texts['request_link_text'].format(link=link)
        await query.message.edit_text(link_text, reply_markup=reply_markup)

    elif query.data == "location_history":
        keyboard = [[InlineKeyboardButton(texts['back_btn'], callback_data="main_menu")]]
        reply_markup = InlineKeyboardMarkup(keyboard)
        await query.message.edit_text(texts['location_history'], reply_markup=reply_markup)
